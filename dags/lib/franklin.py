...
def attach_vcf_to_analysis(clin_s3: S3Hook, analysis: dict, vcfs: dict[str, dict],
    ...
    logging.info(f'Attach VCF for aliquot {aliquot_id} in family {family_id}. VCF bucket: {vcf_bucket}. VCF key: {vcf_key}.')
    return analysis
...
def can_create_analysis(clin_s3, analyses):
    for analysis in analyses:
        ...
        completed_analysis_keys = clin_s3.list_keys(clin_datalake_bucket,
                                                    build_s3_analyses_root_key(analysis_id, aliquot_id))
...
def build_s3_analyses_root_key(analysis_id, aliquot_id):
    return f'raw/landing/franklin/analysis_id={analysis_id}/aliquot_id={aliquot_id or "null"}'


def build_s3_analyses_json_key(analysis_id, aliquot_id, franklin_analysis_id):
    return f'{build_s3_analyses_root_key(analysis_id, aliquot_id)}/franklin_analysis_id={franklin_analysis_id}/analysis.json'


def build_s3_analyses_status_key(analysis_id, aliquot_id):
    return f'{build_s3_analyses_root_key(analysis_id, aliquot_id)}/_FRANKLIN_STATUS_.txt'


def build_s3_analyses_id_key(analysis_id, aliquot_id):
    return f'{build_s3_analyses_root_key(analysis_id, aliquot_id)}/_FRANKLIN_ID_.txt'


def build_s3_analyses_ids_key(analysis_id):
    return f'raw/landing/franklin/analysis_id={analysis_id}/_FRANKLIN_IDS_.txt'
...
def build_create_analysis_payload(family_id, analyses, franklin_s3):
    family_analyses = []
    analyses_payload = []
    ...
    for analysis in analyses:
        ...
        vcf_franklin_s3_full_path = f'{env}/{vcf_key}'
...
def post_create_analysis(family_id, analyses, token, franklin_s3):
    conn = get_franklin_http_conn()
