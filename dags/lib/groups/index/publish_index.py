...
def publish_index(
    ...
    """
    gene_centric = publish.gene_centric(release_id('gene_centric'), color, spark_jar)
    gene_suggestions = publish.gene_suggestions(release_id('gene_suggestions'), color, spark_jar)
    variant_centric = publish.variant_centric(release_id('variant_centric'), color, spark_jar)
    variant_suggestions = publish.variant_suggestions(release_id('variant_suggestions'), color, spark_jar)
    cnv_centric = publish.cnv_centric(release_id('cnv_centric'), color, spark_jar, skip=skip_cnv_centric)
    coverage_by_gene_centric = publish.coverage_by_gene_centric(release_id('coverage_by_gene_centric'), color, spark_jar)
