...
# TODO: enable reporting options automatically via default nextflow.config
def annotate_variants(task_id: str,
    ...
    return nextflow_base_config \
        .append_args(
            *get_run_pipeline_arguments(
                pipeline=variant_annotation_pipeline
            ),
            '--input', input,
            '--outdir', outdir
         ) \
        .with_config_maps(variant_annotation_pipeline.config_maps) \
        .operator(
            task_id=task_id,
            name=task_id,
            skip=skip,
