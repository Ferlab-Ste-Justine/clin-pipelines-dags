...
@task.virtualenv(skip_on_exit_code=SKIP_EXIT_CODE, task_id='prepare_post_processing', requirements=["deltalake===0.24.0"], inlets=[enriched_clinical])
def prepare(analysis_id_pheno_file_mapping: Dict[str, str], job_hash: str, skip: str) -> str:
    ...
    # snv_vcf_germline_urls (gvcf) is a list of URLs, we only need the first one
    # Nextflow only supports s3:// URLs
    samples['gvcf'] = samples['gvcf'].str[0].str.replace('s3a://', 's3://', 1)
    samples['familyPheno'] = samples['familyId'].map(analysis_id_pheno_file_mapping)
