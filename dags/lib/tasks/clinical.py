...
def get_all_analysis_ids(sequencing_ids: Optional[Set[str]] = None, batch_ids: Optional[Set[str]] = None) -> List[str]:
    ...
    all_analysis_ids = sorted(get_analysis_ids(clinical_df, sequencing_ids, batch_ids))

    if not all_analysis_ids:
        raise AirflowFailException(f"No analysis IDs found for the provided sequencing IDs ({sequencing_ids}) or batch IDs ({batch_ids}).")
