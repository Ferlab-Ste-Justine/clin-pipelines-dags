...
def detect(batch_id: Optional[str] = None, sequencing_ids: Optional[List[str]] = None) -> Dict[str, str]:
    ...
    # TODO: validate if this is the behaviour that we want
...
def _detect_type_from_enrich_clinical(identifier_column, identifiers, must_exist=True):
    ...
    code_to_type: dict = {
        BioinfoAnalysisCode.GEBA.value: ClinAnalysis.GERMLINE.value,
        BioinfoAnalysisCode.TEBA.value: ClinAnalysis.SOMATIC_TUMOR_ONLY.value,
        BioinfoAnalysisCode.TNEBA.value:  ClinAnalysis.SOMATIC_TUMOR_NORMAL.value
    }

    df: DataFrame = to_pandas(enriched_clinical.uri)

    distinct_pairs_df = (
        df[[identifier_column, "bioinfo_analysis_code"]]
    ...
    for tup in distinct_pairs_df.itertuples():
        identifier_to_codes[tup[0]].append(tup[1])
    ...
    identifiers_with_unknown_codes = {k: v for k, v in identifier_to_codes.items() if v[0] not in code_to_type}
    ...
    identifier_to_type = {k: code_to_type[v[0]] for k, v in identifier_to_codes.items()}
    return identifier_to_type
